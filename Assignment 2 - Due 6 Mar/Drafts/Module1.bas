Attribute VB_Name = "Module1"
    ...
    'ColCodes(5)  (Volume)          should be a positive number (can't enforce integer status due to value range)
    ...
    'Returns a simple True or False
        ...
        With SheetName
            ...
            ValidCheck(2) = IsNumeric("-" & .Cells(RowNum, ColCodes(2)))    'True is date is a number
                'Actual date formats not supported, uses the leading "-" hack to fix VBA IsNumeric yields true when empty issue
        ...
        Validate_Row = ValidCheck(1) And ValidCheck(2) And ValidCheck(3) And ValidCheck(4) And ValidCheck(5)
...
Sub Stock_Analyze()
    ...
    Dim Header_Row As Long, Ticker_Block_Start As Long, Ticker_Block_End As Long, Output_Row As Long
    ...
    For Each Current_Sheet In Worksheets
        With Current_Sheet
            ...
            Last_Row = .UsedRange.Rows.Count            'calls to .UsedRange seem to slow down the macro a lot
            ...
            For Row_Index = Header_Row + 1 To Last_Row + 1
                ...
                If Validate_Row(Current_Sheet, Row_Index, Label_Array) = True Or Row_Index = Last_Row + 1 Then
                ...
                'We need to enter this branch on the last row to write the final entry in the data table
                    ...
                    ElseIf Current_Ticker <> CStr(.Cells(Row_Index, Ticker_Col).Value) Then
                        ...
                        Set Date_Range = .Range(.Cells(Ticker_Block_Start, Date_Col), .Cells(Ticker_Block_End, Date_Col))
                        ...
                        End If
                        Set Vol_Range = .Range(.Cells(Ticker_Block_Start, Vol_Col), .Cells(Ticker_Block_End, Vol_Col))
                    ...
                    Else    'This is the case where data is valid but ticker didn't change
                            'All we have to do here is expand the block range
                        Ticker_Block_End = Row_Index
                ...
                Else    'Now handle the case where the row data is invalid.
                    ...
                    'Because we do not expect other calculations on the sheet, we won't worry abut undoing these changes
            ...
            If Output_Row > 1 Then
                ...
                Set Summary_Search_Range_Ticker = .Range(.Cells(2, Output_Column), .Cells(Output_Row, Output_Column))
                Set Summary_Search_Range_Percent = .Range(.Cells(2, Output_Column + 2), .Cells(Output_Row, Output_Column + 2))
                Set Summary_Search_Range_Vol = .Range(.Cells(2, Output_Column + 3), .Cells(Output_Row, Output_Column + 3))
                ...
                If Max_Val > 0 Then   'Check that is really is an increase
                    Max_Val_Index = WorksheetFunction.Match(Max_Val, Summary_Search_Range_Percent, 0)  'Identify the corresponding row
                    'Uses the Index function to retrieve the ticker symbol in the corresponding row and write it
                    .Cells(2, Output_Column + 1).Value = WorksheetFunction.Index(Summary_Search_Range_Ticker, Max_Val_Index)
                    .Cells(2, Output_Column + 2).Value = Format(Max_Val, "Percent")    'Write the maximum value
                Else
                ...
                If Min_Val < 0 Then   'Check that is really is a decrease
                    Min_Val_Index = WorksheetFunction.Match(Min_Val, Summary_Search_Range_Percent, 0)  'Identify the corresponding row
                    'Uses the Index function to retrieve the ticker symbol in the corresponding row
                    .Cells(3, Output_Column + 1).Value = WorksheetFunction.Index(Summary_Search_Range_Ticker, Min_Val_Index)
                ...
                Max_Val_Index = WorksheetFunction.Match(Max_Val, Summary_Search_Range_Vol, 0)  'Identify the corresponding row
                'Uses the Index function to retrieve the ticker symbol in the corresponding row and write it
                .Cells(4, Output_Column + 1).Value = WorksheetFunction.Index(Summary_Search_Range_Ticker, Max_Val_Index)
